...
from .delete_shift.delete_shift_controller import router as delete_shift_router
...
from .get_my_shifts.get_my_shifts_controller import router as get_my_shifts_router
from .get_my_hours.get_my_hours_controller import router as get_my_hours_router


# Combine all shift routers
router = APIRouter(prefix="/api/v1", tags=["shifts"])

# Include all feature routers (/shifts/me* before /shifts/{id})
router.include_router(get_my_hours_router)
router.include_router(get_my_shifts_router)
...
router.include_router(delete_shift_router)
